...
Пример предметной модели в духе Domain Driven Design: кинотеатр с залами, сеансами, билетами и деньгами.

## Зачем это нужно

Показать, как раскладывается предметная область, когда она перестаёт помещаться в один файл. Слои разведены намеренно:

- `models/` — сущности предметной области с их правилами;
- `repositories/` — хранение и поиск, отдельно от правил;
- `services/` — сценарии, которые складывают сущности в осмысленное действие («продать билет», «создать сеанс»);
- `lib/` — общая обвязка.

Ценность примера в границах между слоями: сущность не знает, где её хранят, а сценарий не лезет внутрь сущности.
